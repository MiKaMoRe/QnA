...
RSpec.describe Vote, type: :model do
  describe '#liked_from?' do
    ...
    before { create(:vote, voteable: resource, author: user) }
  ...
  describe '#vote_from' do
    ...
    let(:user) { create(:user) }
    let!(:vote) { create(:vote, voteable: resource, author: user) } 
    ...
    context 'when user dont like question' do
      it { expect(resource.vote_from(user).id).not_to eq create(:vote, voteable: resource).id }
    end
  end
  ...
  describe '#total_votes' do
    ...
    context 'when question got up votes and down votes' do
      before do
        create(:vote, voteable: resource)
        create(:vote, voteable: resource)
        create(:vote, voteable: resource, liked: false)
      end
